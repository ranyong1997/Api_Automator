...
from common.mysql_server import db
...
def step_last():
    logger.info("后置条件 ===>> 清理数据")


@allure.step("前置步骤 ===>> 管理员用户登录")
def step_login(username, password):
    logger.info("前置步骤 ===>> 管理员 {} 登录，返回信息 为: {}".format(username, pasword))
...
def delete_register_user():
    """
    注册用户前，先删除数据，用例执行之后，再删除以清理数据
    """
    del_sql = base_data["init_sql"]["delete_register_user"]
    db.excute_db(del_sql)
    step_first()
    logger.info("注册用户操作: 清理用户--准备用户数据")
...
def update_user_telephone():
    """
    修改用户前，因为手机号是唯一，为了使用例重复执行，每次需要先修改手机，在执行用例
    """
    update_sql = base_data["init_sql"]["update_user_telephone"]
    db.excute_db(update_sql)
    step_first()
